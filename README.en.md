...
<p><strong>A self-hosted, multi-tenant knowledge agent platform</strong><br/>Bring RAG and multi-agent execution into one workspace</p>
...
Yuxi is a **self-hosted, multi-tenant knowledge agent platform**. Rather than stopping at a chat interface, it brings **RAG retrieval, LangGraph multi-agent orchestration, MCP/Skills, sandbox tools, and access control** into one workspace.

Administrators connect model providers, build knowledge bases, and manage user or department permissions. Users work with agents that can retrieve cited sources, call tools and sub-agents, and deliver previewable, downloadable artifacts.
...
| Storage | PostgreSQL · Redis · RustFS · Milvus |
...
Third-party components brought in by Docker Compose retain their original licenses; see the [deployment guide](docs/advanced/deployment.md) for deployment and redistribution boundaries.
