...
Yuxi 是一个可私有部署的多租户知识智能体平台。它把知识库检索、LangGraph 多智能体编排、MCP/Skills、沙盒工具和权限管理放进同一个工作区。
...
| 存储     | PostgreSQL · Redis · RustFS · Milvus        |
...
根据文件层级和主题元数据自动生成知识导图，用来浏览知识库文件结构，不代替 chunk 检索。

- 结合知识库文件元数据生成多层级导图。
- 新增文件可以增量更新；纯删除可以直接移除对应叶子节点。
- Agent 可以通过 `get_mindmap` 读取已生成的导图。
