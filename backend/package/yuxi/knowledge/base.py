...
class KnowledgeBase(ABC):
    ...
    async def add_file_record(
        ...
        *,
        ...
        if metadata.get("size") is None and content_type == "file":
            try:
                from yuxi.knowledge.utils.kb_utils import is_object_store_url, parse_object_store_url
                from yuxi.storage.s3 import get_object_store_client

                file_path = metadata.get("path") or item
                if is_object_store_url(file_path):
                    bucket_name, obj_name = parse_object_store_url(file_path)
                    object_store = get_object_store_client()
                    file_size = await object_store.astat_file(bucket_name, obj_name)
    ...
    async def parse_file(
        ...
        *,
        ...
        try:
            ...
            params["image_bucket"] = get_object_store_client().KB_BUCKETS["images"]
            ...
            markdown_file_path = await self._save_markdown_to_object_store(kb_id, file_id, markdown_content)
    ...
    async def _save_markdown_to_object_store(self, kb_id: str, file_id: str, content: str) -> str:
        """Save markdown content to MinIO and return HTTP URL"""
        from yuxi.storage.s3 import get_object_store_client

        object_store = get_object_store_client()
        bucket_name = object_store.KB_BUCKETS["parsed"]
        await asyncio.to_thread(object_store.ensure_bucket_exists, bucket_name)

        object_name = f"{kb_id}/parsed/{file_id}.md"
        data = content.encode("utf-8")

        # Return standard HTTP URL from UploadResult
        upload_result = await object_store.aupload_file(
    ...
    async def _read_object_bytes(self, file_path: str) -> bytes:
        from yuxi.knowledge.utils.kb_utils import is_object_store_url, parse_object_store_url
        from yuxi.storage.s3 import get_object_store_client

        if not file_path or not is_object_store_url(file_path):
            raise ValueError(f"Invalid MinIO path format: {file_path}")

        bucket_name, object_name = parse_object_store_url(file_path)
        object_store = get_object_store_client()
        return await object_store.adownload_file(bucket_name, object_name)

    async def _read_markdown_from_object_store(self, file_path: str) -> str:
        """Read markdown content from MinIO"""
        content_bytes = await self._read_object_bytes(file_path)
    ...
    async def get_file_download(self, kb_id: str, file_id: str, variant: str = "original") -> dict:
        ...
        if variant == "parsed":
            ...
            return {
                ...
                "content": await self._read_object_bytes(markdown_file),
        ...
        return {
            "filename": filename,
            "content": await self._read_object_bytes(original_path),
    ...
    async def open_file_content(self, kb_id: str, file_id: str, offset: int = 0, limit: int = 800) -> dict:
        ...
        content = await self._read_markdown_from_object_store(markdown_file)
    ...
    async def cleanup_database_resources(self, kb_id: str) -> dict:
        ...
        while True:
            ...
            for record in records:
                ...
                if file_path and is_object_store_url(file_path):
                    try:
                        bucket_name, object_name = parse_object_store_url(file_path)
                        await object_store.adelete_file(bucket_name, object_name)
                    except Exception as e:
                        logger.warning(f"Failed to delete MinIO file {file_path}: {e}")

                # 删除解析后的 markdown 文件
                parsed_object = f"{kb_id}/parsed/{file_id}.md"
                await object_store.adelete_file(object_store.KB_BUCKETS["parsed"], parsed_object)

        # 2. 并行删除所有知识库 bucket 中该 kb_id 下的文件
        prefix = f"{kb_id}/"
        cleanup_buckets = {
            object_store.KB_BUCKETS["parsed"],
            object_store.KB_BUCKETS["documents"],
            object_store.KB_BUCKETS["images"],
        }
        cleanup_tasks = [object_store.adelete_objects_by_prefix(bucket_name, prefix) for bucket_name in cleanup_buckets]
        ...
        async def _stat_file(file_id: str, file_path: str) -> tuple[str, int | None]:
            ...
            try:
                async with semaphore:
                    return file_id, await object_store.astat_file(bucket_name, obj_name)
            except Exception as exc:
