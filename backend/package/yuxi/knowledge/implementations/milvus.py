...
class MilvusKB(KnowledgeBase):
    ...
    async def index_file(
        ...
        *,
        ...
        try:
            ...
            # Read markdown
            markdown_content = await self._read_markdown_from_object_store(file_meta["markdown_file"])
    ...
    async def _get_file_content_from_meta(self, file_id: str, file_meta: dict) -> dict:
        ...
        if file_meta.get("markdown_file"):
            try:
                content = await self._read_markdown_from_object_store(file_meta["markdown_file"])
    ...
    async def cleanup_database_resources(self, kb_id: str) -> dict:
        ...
        def delete_milvus_collections() -> None:
            ...
            for suffix in ("_entity", "_triple"):
                collection_name = f"{kb_id}{suffix}"
                try:
                    if utility.has_collection(collection_name, using=self.connection_alias):
                        utility.drop_collection(collection_name, using=self.connection_alias)
                except Exception as e:
                    logger.error(f"Failed to drop leftover graph collection {collection_name}: {e}")
