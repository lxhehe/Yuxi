...
async def parse_resolved_document(source: str, params: dict | None = None) -> str:
    ...
    from yuxi.storage.s3.client import get_object_store_client

    # 1. 如果是 MinIO URL，下载文件到临时路径
    if is_object_store_url(source):
        ...
        try:
            bucket_name, object_name = parse_object_store_url(source)
    ...
    try:
        ...
        if is_object_store_url(source) and os.path.exists(actual_file_path):
            try:
...
def _resolve_image_storage_params(params: dict | None) -> tuple[str, str]:
    ...
    image_bucket = params.get("image_bucket") or get_object_store_client().KB_BUCKETS["images"]
...
def _upload_image_to_object_store(image_data: bytes, filename: str, bucket_name: str, object_prefix: str) -> str:
    """上传图片到 MinIO，返回经后端鉴权代理访问的 URL。"""
    from yuxi.knowledge.utils.kb_utils import build_kb_image_proxy_url

    object_store = get_object_store_client()
    object_store.ensure_bucket_exists(bucket_name)
        ...
        for pic in doc.pictures:
            ...
            if uri.startswith("data:"):
                ...
                try:
                    ...
                    url = _upload_image_to_object_store(image_data, filename, image_bucket, image_prefix)
