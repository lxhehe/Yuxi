...
async def read_knowledge_file_preview(kb_id: str, file_id: str) -> dict:
    ...
    raw_content = await _read_object_bytes(original_path)
...
async def _read_office_pdf_preview(
    ...
    if await object_store.astat_file(bucket_name, object_name) is not None:
    ...
    await object_store.aupload_file(
...
async def _get_minio_file_size(file_path: str) -> int | None:
    ...
    return await get_object_store_client().astat_file(bucket_name, object_name)


async def _read_object_bytes(file_path: str) -> bytes:
    bucket_name, object_name = _parse_minio_path(file_path)
    return await get_object_store_client().adownload_file(bucket_name, object_name)
...
def _parse_minio_path(file_path: str) -> tuple[str, str]:
    ...
    return parse_object_store_url(file_path)
