...
async def prepare_item_metadata(item: str, content_type: str, kb_id: str, params: dict | None = None) -> dict:
    ...
    if content_type == "file":
        if not is_object_store_url(item):
            raise ValueError(f"File source must be an object store URL: {item}")

        logger.debug(f"Processing object store file: {item}")
        _, object_name = parse_object_store_url(item)
...
def is_object_store_url(file_path: str) -> bool:
    """检测是否是本系统生成的对象存储 URL。"""
    from urllib.parse import urlparse

    parsed_url = urlparse(file_path)
    if parsed_url.scheme in {"s3", "minio"}:
    ...
    from yuxi.storage.s3.client import ObjectStoreClient

    known_buckets = set(ObjectStoreClient.KB_BUCKETS.values()) | ObjectStoreClient.PUBLIC_READ_BUCKETS
    return path_parts[0] in known_buckets


def parse_object_store_url(file_path: str) -> tuple[str, str]:
    """解析对象存储 URL，提取 bucket 与 object name。

    支持：
    - s3://bucket/key
    - minio://bucket/key（历史内部 URL）
    ...
    try:
        ...
        if parsed_url.scheme in {"s3", "minio"}:
            ...
            if len(path_parts) > 1:
                ...
                raise ValueError(f"无法解析对象 URL 中的 bucket 名称: {file_path}")

        logger.debug(f"Parsed object URL: bucket_name={bucket_name}, object_name={object_name}")
        return bucket_name, object_name

    except Exception as e:
        logger.error(f"Failed to parse object URL {file_path}: {e}")
        raise ValueError(f"无法解析对象 URL: {file_path}")
