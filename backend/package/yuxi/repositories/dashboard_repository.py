...
from yuxi.storage.s3.client import normalize_public_object_url
            ...
            items.append(
                {
                    ...
                    "user_avatar": normalize_public_object_url(user.avatar) if user and user.avatar else None,
                    "user_deleted": user is None or bool(user.is_deleted),
                    "agent_id": conversation.agent_id,
                    "agent_name": agent.name if agent else conversation.agent_id,
                    "agent_avatar": normalize_public_object_url(agent.icon) if agent and agent.icon else None,
        ...
        users = [
            {
                ...
                "avatar": normalize_public_object_url(row.avatar) if row.avatar else None,
        ...
        agents = [
            {
                ...
                "avatar": normalize_public_object_url(row.icon) if row.icon else None,
    ...
    async def get_thread_analytics(
        ...
        *,
        ...
        if agent_slugs:
            ...
            agent_avatars_map = {a.slug: normalize_public_object_url(a.icon) if a.icon else None for a in agents}
