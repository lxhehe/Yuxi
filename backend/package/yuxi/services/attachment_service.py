...
from yuxi.storage.s3 import StorageError, get_object_store_client
...
def _object_store_source(bucket_name: str, object_name: str) -> str:
    return f"s3://{bucket_name}/{quote(object_name, safe='/')}"
...
async def _cleanup_expired_tmp_attachments(object_store, bucket_name: str, uid: str) -> None:
    """上传时顺手清理当前用户 24 小时前遗留的临时附件。"""
    prefix = f"{TMP_ATTACHMENT_PREFIX}/{uid}/"
    try:
        objects = await object_store.alist_object_metadata(bucket_name, prefix)
    ...
    results = await asyncio.gather(
        *(
            object_store.adelete_objects_by_prefix(bucket_name, f"{_tmp_attachment_prefix(uid, tmp_file_id)}/")
...
async def upload_tmp_attachment_view(*, file: UploadFile, current_uid: str) -> dict:
    ...
    bucket_name = object_store.KB_BUCKETS["documents"]
    try:
    ...
    await _cleanup_expired_tmp_attachments(object_store, bucket_name, str(current_uid))
...
async def parse_tmp_attachment_view(
    *,
    ...
    try:
        ...
        markdown = await parse_document(_object_store_source(bucket_name, object_name), params={"ocr_engine": method})
    ...
    *,
    ...
    try:
        for item in attachments:
            ...
            try:
                file_content = await object_store.adownload_file(bucket_name, object_name)
            except StorageError as exc:
                raise HTTPException(status_code=400, detail=f"读取临时附件失败: {exc}") from exc
            ...
            if parsed_object_name:
                ...
                try:
                    parsed_bytes = await object_store.adownload_file(bucket_name, parsed_object_name)
    ...
    delete_results = await asyncio.gather(
        *(
            object_store.adelete_objects_by_prefix(
                bucket_name,
