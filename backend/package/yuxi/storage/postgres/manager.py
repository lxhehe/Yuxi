...
KNOWLEDGE_GRAPH_QUERY_PARAM_KEYS = (
    "use_graph_retrieval",
    "graph_entity_top_k",
    "graph_triple_top_k",
    "graph_max_nodes",
...
KNOWLEDGE_SCHEMA_V3_STATEMENTS = (
    """
    UPDATE tasks
    SET status = 'failed',
        error = '知识图谱能力已移除',
        ...
        heartbeat_at = NULL
    WHERE type = 'knowledge_graph_index'
      AND status NOT IN ('success', 'failed', 'cancelled')
    """,
    "DROP TABLE IF EXISTS knowledge_graph_triple_mentions",
    "DROP TABLE IF EXISTS knowledge_graph_entity_mentions",
    "DROP TABLE IF EXISTS knowledge_graph_triples",
    "DROP TABLE IF EXISTS knowledge_graph_entities",
    "DROP INDEX IF EXISTS ix_knowledge_chunks_graph_indexed",
    "DROP INDEX IF EXISTS ix_knowledge_chunks_graph_structure_indexed",
    "DROP INDEX IF EXISTS ix_knowledge_chunks_graph_extraction_status",
    "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS graph_indexed",
    "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS graph_structure_indexed",
    "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS graph_extraction_details",
    "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS ent_ids",
    "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS tags",
    "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS extraction_result",
    """
    UPDATE knowledge_files
    SET path = regexp_replace(path, '^minio://', 's3://')
    WHERE path LIKE 'minio://%'
    """,
    """
    UPDATE knowledge_files
    SET markdown_file = regexp_replace(markdown_file, '^minio://', 's3://')
    WHERE markdown_file LIKE 'minio://%'
    """,
    """
    UPDATE knowledge_files
    SET minio_url = regexp_replace(minio_url, '^minio://', 's3://')
    WHERE minio_url LIKE 'minio://%'
    """,
    """
    UPDATE knowledge_bases
    SET additional_params = additional_params - 'graph_build_config'
    WHERE additional_params ? 'graph_build_config'
    """,
    f"""
    UPDATE knowledge_bases
    SET query_params = query_params - '{ "' - '".join(KNOWLEDGE_GRAPH_QUERY_PARAM_KEYS) }'
...
class PostgresManager(metaclass=SingletonMeta):
    ...
    async def upgrade_knowledge_schema_v2_to_v3(self) -> None:
        """删除知识图谱存储并改写对象 URL scheme。"""
        self._check_initialized()
        async with self.async_engine.begin() as conn:
            for statement in KNOWLEDGE_SCHEMA_V3_STATEMENTS:
                await conn.execute(text(statement))
