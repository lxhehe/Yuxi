...
from yuxi.knowledge.utils import calculate_content_hash, is_object_store_url, params_for_uploaded_document, parse_object_store_url
...
from yuxi.storage.s3.client import ObjectStoreClient, StorageError, aupload_file_to_object_store, get_object_store_client
...
async def _delete_document_storage_objects(kb_id: str, doc_id: str, file_path: str) -> None:
    ...
    try:
        await object_store.adelete_file(object_store.KB_BUCKETS["parsed"], f"{kb_id}/parsed/{doc_id}.md")
    except Exception as minio_error:
        logger.warning(f"从MinIO删除解析结果失败: {minio_error}")

    try:
        await object_store.adelete_file(object_store.KB_BUCKETS["parsed"], f"{kb_id}/preview/{doc_id}.pdf")
    except Exception as minio_error:
...
def _validate_uploaded_document_items(items: list[str], params: dict) -> None:
    ...
    for item in items:
        ...
        if not is_object_store_url(item):
            raise HTTPException(status_code=400, detail="File source must be an object store URL")
...
async def download_document(kb_id: str, doc_id: str, current_user: User = Depends(require_knowledge_base_read)):
    ...
    try:
        ...
        if not is_object_store_url(file_path):
            raise HTTPException(status_code=400, detail="文件路径必须是对象存储 URL")
        ...
        try:
            ...
            # 直接使用解析出的完整对象名称下载
            minio_response = await object_store.adownload_response(
...
async def get_kb_image(kb_id: str, object_path: str, current_user: User = Depends(require_knowledge_base_read)):
    ...
    try:
        minio_response = await object_store.adownload_response(
            bucket_name=ObjectStoreClient.KB_BUCKETS["images"],
            object_name=object_name,
        )
...
async def fetch_url(
    ...
    try:
        ...
        # 3. 上传到 MinIO
        object_store = get_object_store_client()
        bucket_name = ObjectStoreClient.KB_BUCKETS["documents"]
...
async def import_workspace_files(
    ...
    for workspace_path in paths:
        ...
        minio_url = await aupload_file_to_object_store(bucket_name, object_name, file_bytes)
