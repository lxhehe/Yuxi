...
def test_object_store_url_accepts_s3_and_legacy_minio():
    assert is_object_store_url("s3://knowledgebases/db/upload/a.txt")
    assert is_object_store_url("minio://knowledgebases/db/upload/a.txt")
    assert is_object_store_url("http://rustfs:9000/knowledgebases/db/upload/a.txt")
    assert not is_object_store_url("/tmp/a.txt")
    assert not is_object_store_url("https://example.com/docs/a.txt")


def test_parse_object_store_url_reads_s3_and_legacy_schemes():
    assert parse_object_store_url("s3://knowledgebases/db/upload/a.txt") == (
        "knowledgebases",
        "db/upload/a.txt",
    )
    assert parse_object_store_url("minio://knowledgebases/db/upload/a.txt") == (
        "knowledgebases",
        "db/upload/a.txt",
    )
    assert parse_object_store_url("http://rustfs:9000/knowledgebases/db/upload/a.txt") == (
        "knowledgebases",
        "db/upload/a.txt",
    )
