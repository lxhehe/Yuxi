...
async def test_read_office_pdf_preview_converts_and_caches_pdf(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    object_store.objects[("knowledgebases", "db1/upload/demo.docx")] = b"office"
    convert_calls = 0
    ...
    monkeypatch.setattr(preview, "get_object_store_client", lambda: object_store)
    ...
    assert object_store.objects[("knowledgebases", "db1/preview/file1.pdf")] == b"%PDF-1.4\nconverted"
...
async def test_non_docx_pptx_office_files_do_not_get_pdf_preview(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    object_store = FakeObjectStoreClient()
    object_store.objects[("knowledgebases", "db1/upload/demo.docx")] = b"PK\x03\x04excel"
    monkeypatch.setattr(preview, "get_object_store_client", lambda: object_store)
...
async def test_read_binary_preview_uses_complete_renderer_result(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    object_store = FakeObjectStoreClient()
    content = b"%PDF-1.4\nreport"
    object_store.objects[("knowledgebases", "db1/upload/demo.docx")] = content
    monkeypatch.setattr(preview, "get_object_store_client", lambda: object_store)
...
async def test_read_file_preview_rejects_large_original_before_download(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fail_object_store_access():
        raise AssertionError("large preview should not access MinIO")

    monkeypatch.setattr(preview, "get_object_store_client", fail_object_store_access)
