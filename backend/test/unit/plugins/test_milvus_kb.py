...
async def test_cleanup_database_resources_offloads_milvus_cleanup(monkeypatch):
    ...
    assert calls[0] == "has_collection"
    assert "drop_collection" in calls
    assert calls[-1] == "delete_base"
...
def test_build_chunk_pg_records_keeps_core_fields():
    kb = MilvusKB.__new__(MilvusKB)
    ...
    assert "extraction_result" not in records[0]
...
async def test_index_file_persists_chunk_stats(monkeypatch):
    ...
    kb._get_or_create_milvus_collection = get_collection
    kb._read_markdown_from_object_store = read_markdown
...
async def test_cancellation_marks_file_retryable(monkeypatch, operation, expected_status, expected_message):
    ...
    if operation == "parse":
        ...
        kb._read_markdown_from_object_store = cancelled_step
