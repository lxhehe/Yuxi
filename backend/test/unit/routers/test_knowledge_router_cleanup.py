...
async def test_upload_file_fails_before_read_or_minio(monkeypatch, kb_id, status_code, error_detail):
    ...
    async def fake_upload_to_object_store(*_args, **_kwargs) -> str:
        calls["upload"] += 1
    ...
    monkeypatch.setattr(knowledge_router, "aupload_file_to_object_store", fake_upload_to_object_store)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "File source must be an object store URL",
        ),
        (
            knowledge_router.AddUploadedDocumentsRequest(
