...
async def test_import_workspace_files_uploads_workspace_file_to_minio(monkeypatch):
    ...
    async def fake_upload(bucket_name: str, file_name: str, data: bytes) -> str:
        assert bucket_name == knowledge_router.ObjectStoreClient.KB_BUCKETS["documents"]
    ...
    monkeypatch.setattr(knowledge_router, "aupload_file_to_object_store", fake_upload)
    ...
    assert item["file_path"].startswith(
        f"s3://{knowledge_router.ObjectStoreClient.KB_BUCKETS['documents']}/db_1/upload/note_"
    )
