...
@pytest.mark.asyncio
async def test_ensure_knowledge_schema_drops_graph_tables_and_rewrites_object_urls():
    async with _recording_manager() as (manager, connection):
        await manager.ensure_knowledge_schema()

    statements = "\n".join(connection.statements)

    assert "DROP TABLE IF EXISTS knowledge_graph_entities" in statements
    assert "ALTER TABLE IF EXISTS knowledge_chunks DROP COLUMN IF EXISTS graph_indexed" in statements
    assert "regexp_replace(path, '^minio://', 's3://')" in statements
    assert "WHERE type = 'knowledge_graph_index'" in statements
