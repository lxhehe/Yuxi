...
async def test_upload_tmp_attachment_writes_user_scoped_minio_object(monkeypatch):
    fake_object_store = FakeObjectStoreClient()
    monkeypatch.setattr(service, "get_object_store_client", lambda: fake_object_store)
    ...
    assert fake_object_store.objects[("knowledgebases", response["object_name"])] == b"pdf-bytes"
...
async def test_upload_tmp_attachment_cleans_only_expired_user_tmp_groups(monkeypatch):
    ...
    fake_object_store.object_metadata = [
        {
    ...
    assert fake_object_store.deleted_prefixes == [("knowledgebases", "tmp/chat_attachments/user-1/expired/")]
...
async def test_parse_tmp_attachment_uses_selected_method_and_uploads_markdown(monkeypatch):
    ...
    fake_object_store.objects[("knowledgebases", object_name)] = b"pdf-bytes"
    monkeypatch.setattr(service, "get_object_store_client", lambda: fake_object_store)
    ...
    assert parse_calls == [
        {
            "source": f"s3://knowledgebases/{object_name}",
            "params": {"ocr_engine": "disable"},
        }
    ]
    assert response["parsed_object_name"] == "tmp/chat_attachments/user-1/tmp-1/parsed/demo.md"
    assert fake_object_store.objects[("knowledgebases", response["parsed_object_name"])] == b"# parsed"
...
async def test_confirm_tmp_thread_attachments_writes_realtime_workdir(confirm_attachment_env):
    ...
    fake_object_store.objects[("knowledgebases", original_object)] = b"pdf-bytes"
    fake_object_store.objects[("knowledgebases", parsed_object)] = b"# parsed"
    ...
    assert fake_object_store.deleted_prefixes == [("knowledgebases", "tmp/chat_attachments/user-1/tmp-1/")]
...
async def test_parse_tmp_attachment_uses_object_name_for_type_validation(monkeypatch):
    ...
    fake_object_store.objects[("knowledgebases", object_name)] = b"docx-bytes"
    monkeypatch.setattr(service, "get_object_store_client", lambda: fake_object_store)
...
async def test_parse_tmp_attachment_handles_url_metacharacters(monkeypatch):
    ...
    assert parse_calls == ["s3://knowledgebases/tmp/chat_attachments/user-1/tmp-1/original/q1%3F.pdf"]
...
async def test_confirm_tmp_thread_attachments_validates_batch_before_commit(confirm_attachment_env):
    ...
    fake_object_store.objects[("knowledgebases", valid_object)] = b"pdf-bytes"
...
async def test_confirm_tmp_thread_attachments_keeps_duplicate_names_separate(confirm_attachment_env):
    ...
    fake_object_store.objects[("knowledgebases", first_object)] = b"first"
    fake_object_store.objects[("knowledgebases", second_object)] = b"second"
