...
生产 Compose 默认不把 PostgreSQL、RustFS 和 Milvus 管理端口发布到公网。维护这些服务时，优先使用 `docker compose exec` 或受控的内网入口。
...
S3_ACCESS_KEY=<strong-s3-access-key>
S3_SECRET_KEY=<strong-s3-secret-key>
...
- PostgreSQL 数据目录；
- 对象存储数据目录（旧 MinIO 卷或新 RustFS 卷）；
- `docker/volumes/yuxi` 中的历史文件、UserWorkspace 和 Skill 数据；
- 当前 Compose、`.env.prod` 和目标版本代码。

备份后至少做一次成套恢复演练。只恢复数据库或只恢复文件卷，会让数据库记录与文件字节不一致。本版本把默认对象存储从 MinIO 换成 RustFS，磁盘格式不兼容；切流量前用 `scripts/copy_object_store.py` 复制对象，不能把旧 MinIO 目录挂到 RustFS。
...
公开头像和智能体图片通过同源 `/minio/public/...` 只读代理访问。这是历史路径合同，不表示仍在运行 MinIO。不要把 RustFS 的 9000 对象 API 或 9001 控制台暴露到公网；知识库等私有 bucket 不经过该代理。需要单独的静态资源域名时，设置 `S3_PUBLIC_BASE_URL`，并在域名侧保持同样的只读限制。已有部署不能把旧 MinIO 数据目录挂到 RustFS；用 `scripts/copy_object_store.py` 复制 `public`、`knowledgebases`、`kb-images` 和 Milvus 使用的 bucket，对账对象数并做一次检索后再切流量。RustFS 容器用户是 UID 10001，bind mount 必须可写。
...
更换 `.env.prod` 中的 PostgreSQL 或 RustFS 凭据，不会自动修改已经写入数据卷的服务凭据。请先使用对应服务的官方管理流程修改数据卷内的凭据，再更新 `.env.prod`，重新创建相关服务，并用旧凭据验证登录已被拒绝。不要把真实密码写进命令历史、日志或文档。
...
RustFS 根密钥由 `S3_ACCESS_KEY` / `S3_SECRET_KEY` 注入。完成轮换后，把新值写入 `.env.prod`，再重建依赖这些凭据的服务：

```bash
docker compose --env-file .env.prod -f docker-compose.prod.yml \
  up -d --force-recreate postgres rustfs api worker
```
...
| 组件 | 镜像引用 | 许可证 |
| --- | --- | --- |
| RustFS | `rustfs/rustfs:1.0.0-rc.5` | Apache-2.0 |
...
如果通过 `docker/save_docker_images.sh` 或其他方式向第三方再分发包含非 MIT 软件的镜像，需要保留许可证文本和上游声明，并按对应许可证提供匹配的完整对应源码或有效的书面源码要约。Redis 使用 RSALv2/SSPLv1，再分发前按实际镜像核对义务。以上是工程侧边界，不构成法律意见；再分发、修改组件或对外托管前请让法务按具体版本和交付方式确认。
