...
Yuxi（语析）是一个可私有部署的多租户知识智能体平台。它把知识库检索、智能体编排、工具调用和文件产物放在同一个工作区里，适合需要自己管理数据、模型和权限的团队。
...
- **知识库**：保存文档并提供解析、分块、向量检索和来源信息。Milvus 知识库还支持知识导图；Dify、Notion 是只读连接器。
...
Milvus 知识库可以根据文件列表和元数据生成层次化导图，保存在 `mindmap` 字段中。导图说明文件组织关系，不代替 chunk 检索。
...
PostgreSQL 保存请求、运行、消息、知识库元数据和 LangGraph checkpoint；Redis 负责任务投递、短期事件、取消信号和缓存；RustFS 保存对象文件；Milvus 负责向量检索。

## 部署与沙盒

开发环境和单机部署使用 Docker Compose，默认启动知识库和评估所需的依赖。应用通过 `sandbox-provisioner` 访问动态沙盒，底层可以使用 Docker 或 Kubernetes。`memory` 仅用于测试，不提供真实隔离。
...
- [知识库](./knowledge-base.md)：创建知识库并让智能体检索文档。
